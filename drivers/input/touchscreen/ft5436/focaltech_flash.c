...
static unsigned char CTPM_FW[] = {
    #include "5436_Ref_ASUS_S89116AA1_V13_D01_20161121_app.i"
};
...
static unsigned char CTPM_FW[] = { 0 };
static unsigned char aucFW_PRAM_BOOT[] = { 0 };
...
int  fts_8716_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	FTS_DBG("8716 dw_lenth= %d",dw_lenth);
	if(dw_lenth > 0x10000 || dw_lenth ==0)
	{		
		return -EIO;
	}
	...
	for (i = 0; i < 20; i++) 
	{
		...
		if ((reg_val[0] == 0x87
			&& reg_val[1] == 0x16)) 
		{
			FTS_DBG("[FTS] Step 3: READ CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} 
		else 
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	pr_info("uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", uc_tp_fm_ver, uc_host_fm_ver);
